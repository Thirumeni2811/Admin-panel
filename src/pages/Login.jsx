    ...
    const login = async (e) => {
        ...
        try {
            if (validateForm()) {
                ...
                // Simulating token values (Replace with actual values if needed)
                const accessToken = "dummyAccessToken123";
                const refreshToken = "dummyRefreshToken123";
                const accessTokenExpires = new Date(new Date().getTime() + 60 * 60 * 1000); // 1 hour from now
                const refreshTokenExpires = new Date(new Date().getTime() + 24 * 60 * 60 * 1000); // 1 day from now
